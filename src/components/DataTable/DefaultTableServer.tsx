...
export const DefaultTableServer = ({
  isLoading: isLoadingOverride,
  ...props
}: DefaultTableServerProps) => {
  // Automatically detect loading state from server context
  const serverContext = useDataTableServerContext();
  const isLoading =
    isLoadingOverride ?? serverContext?.query?.isLoading ?? false;

  return <DefaultTable {...props} isLoading={isLoading} />;
};
